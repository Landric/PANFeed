        ...
        for feedurl in feeds:
            ...
            try:
                ...
                for item in feed.entries:
                    if (c.execute("""SELECT * FROM corpus WHERE url = %s AND feed = %s""",(item.link,feedurl))==0):
                        try:
                            d = datetime.datetime(*(item.date_parsed[0:6]))
                        except AttributeError:
                            d=datetime.datetime.now()
                        dStr = d.isoformat(' ')
                        c.execute("""INSERT INTO corpus (title,description,url,feed,length,date) VALUES (%s,%s,%s,%s,%s,%s)""",(item.title,item.description,item.link,feedurl,len(item.description+item.title),dStr))
